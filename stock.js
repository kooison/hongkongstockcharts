...
function createCharts(row, stockNum) {
	var output = "<tr>";
	var newUrl;
	var id;
	if (stockNum != undefined) {
		$.each(CHARTTYPES, function (key, chart) {
			id = row + "_" + key;
			newUrl = chart.url.format(stockNum) + '&chartwidth=' + CONFIG.imgWidth + '&chartheight=' + CONFIG.imgHeight + '&ttt=' + new Date().getTime();
			if (chart.active) {
...
$(document).ready(function () {
	...
	$.each(CONFIG.stockList, function (row, stockNum) {
		$('#stockList > tbody:last-child').append(createCharts(row, stockNum));
	});
